...
module.exports = function(controller) {
    ...
    controller.hears(["Rechart"],'message,direct_message', async(bot,message)=> {
        ...
        await bot.reply(message, 
            { 
                text: `Recharts is a light weight chart visualization library. ${data.firstName} has used it for ${years} ${plural}`,
    ...
    const capitals = new Set(["SQL", "HTML", "CSS"])
    ...
    controller.hears(tech,'message,direct_message', async(bot,message)=> {
        ...
        message.text.split(" ").forEach( word => {
            ...
            if (knownTech.has(word.toLowerCase())) {
                if (capitals.has(word.toUpperCase())) {
                    cur = word.toUpperCase()
                } else {   
                    cur = word;
        ...
        await bot.reply(message, 
            { 
                text: `${data.firstName} has experience with ${cur} for ${years} ${plural}.`,
                quick_replies: data.techs
            }
        )
    })
